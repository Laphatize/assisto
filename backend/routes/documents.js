...
router.post("/process", upload.single("file"), async (req, res) => {
  try {
    ...
    if (!content && !isPdf && !isImage && !structuredHasData) {
...
Return ONLY valid JSON, no markdown.`,
    ...
    if (Array.isArray(parsed?.compliance_entities) || Array.isArray(parsed?.compliance_transactions)) {
